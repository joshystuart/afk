...
export class ContainerLogStreamService implements OnModuleDestroy {
  ...
  /** socket id → session id for an active logs subscription */
  private readonly socketLogSession = new Map<string, string>();

  constructor(private readonly dockerEngine: DockerEngineService) {}

  /**
   * Ensures a single follow stream exists for this session/container pair.
   * Call after the container is running (create, start, recreate).
   */
  ...
  /**
   * Attach a websocket client to the shared stream. Duplicate subscribe from
   * the same socket replaces the callback and moves the socket off any previous
   * session's log subscription.
   */
  addSubscriber(
    sessionId: string,
    containerId: string,
