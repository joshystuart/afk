...
export class DockerEngineService {
    ...
    async createContainer(options: ContainerCreateOptions): Promise<Dockerode.Container> {
        ...
        try {
            ...
            const logStream = await container.logs({ follow: true, stdout: true, stderr: true, tail: 1000 });
            logStream.on('data', (chunk) => {
                this.logger.debug(`Container ${options.sessionId}`, chunk.toString('utf8'));
            });
            logStream.on('end', () => {
                this.logger.debug(`Container ${options.sessionId} - Log stream ended.`);
            });
            logStream.on('error', (err) => {
                this.logger.debug(`Container ${options.sessionId} - Log stream error`, err);
            });
    ...
    private convertToSshUrl(url: string): string {
        ...
        for (const {pattern, replacement} of httpsPatterns) {
            if (pattern.test(url)) {
                ...
                this.logger.log(`Converted HTTPS URL to SSH: ${url} -> ${convertedUrl}`);
                return convertedUrl;
            }
        }

        // Return original URL if no conversion needed
        this.logger.log(`URL already in SSH format or not supported for conversion: ${url}`);
        return url;
    }
